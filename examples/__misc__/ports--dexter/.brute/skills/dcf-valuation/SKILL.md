...
Call the `get_cash_flow_statements` tool with `period: "annual"`, `limit: 5`.
...
### 1.3 Balance Sheet
Call the `get_balance_sheets` tool with `period: "annual"`, `limit: 1`.
...
### 1.5 Company Facts
Call the `get_company_facts` tool.
