...
any_file_match() {
  ...
  # Patterns are literal script constants, never user input. "$REPO" is quoted so
  # a path with spaces is not split; $pattern is left unquoted so it globs. When a
  # glob matches nothing the loop iterates once over the literal pattern, and the
  # -e test fails, so no false pass. (Avoids `compgen`, which is absent from bash
  # builds compiled without --enable-progcomp, e.g. some Nix/minimal environments.)
  local pattern f
  for pattern in "$@"; do
    for f in "$REPO"/$pattern; do
      [ -e "$f" ] && { echo "pass"; return; }
    done
  done
  echo "fail"
}
...
if [[ "$inst" == "pass" ]]; then
  ...
  check_critical "Verification commands are listed in instructions file" \
    "$(contains_pattern "$ipath" "(make check|npm test|mix test|pytest|cargo test|make test|yarn test|bin/test|bundle exec|rake|rspec|verification|verify)")" \
...
check_critical "Verification command documented in AGENTS.md or CLAUDE.md" \
  "$(contains_pattern "$(instructions_path 2>/dev/null || echo "AGENTS.md")" "(make check|npm test|mix test|pytest|cargo test|make test|yarn test|bin/test|bundle exec|rake|rspec|verify|verification)")" \
