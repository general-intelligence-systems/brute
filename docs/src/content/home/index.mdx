...
Head to [Getting Started](./docs/getting-started/) for a complete runnable walkthrough.

## What's here

<CardGrid>
	<Card title="Core Features" icon="puzzle">
		The [agent pipeline](./docs/agents/) (`Brute.agent`, `.use`, `.run`, `.start`),
		the [middleware catalog](./docs/middleware/), Brute's [message
		format](./docs/messages/), the [MessageTransport
		pattern](./docs/message-transports/) with shipped transports for four LLM
		libraries, and the [tool system](./docs/tools/).
	</Card>
	<Card title="Advanced" icon="rocket">
		[Sub-agents](./docs/sub-agents/) (agents as tools), [session
		persistence](./docs/sessions/), [skills](./docs/skills/),
		[events](./docs/events/), and [serving agents over HTTP](./docs/rack/).
	</Card>
