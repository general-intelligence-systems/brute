...
These four build the *identical* agent — same middleware stack, same tools, same task. Only the terminal `run` proc differs, showing the [MessageTransport](../docs/message-transports/) for each library:
...
The ruby_llm version builds `RubyLLM::Tool` classes; the anthropic version emits `{ name:, description:, input_schema: }`. Same source adapter, different shape — see [Tools](../docs/tools/).
...
| `01c-brute-ru/config.ru` | serving that agent over HTTP via [`Brute::Rack::Adapter`](../docs/rack/) |
| `03-session-persistence/` | [`SessionLog`](../docs/sessions/) across turns |
| `05-multi-turn/` | a continuing conversation |
| `06-read-only-agent/` | a restricted tool set |
| `07-subagent-exploration/` | [sub-agents](../docs/sub-agents/) delegating work |
...
See [Serving over HTTP](../docs/rack/) for the request/response details.
